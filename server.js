...
function runApp() {
    ...
    // (async () => {
    //     const tunnel = await lt({ port: 7500 });
    ...
    var server = new MRE.WebHost({
        baseDir: path_1.resolve(__dirname, '../click-game/public'),
        port: 7500
    });

    // Handle new application sessions
    server.adapter.onConnection(function(context) { return new app_1.default(context) });
    //     server.adapter.onConnection(function(context1) { return  });
}
